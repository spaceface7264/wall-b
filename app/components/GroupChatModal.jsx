      ...
      >
          ...
          <div className="p-4 border-b" style={{ borderColor: 'var(--divider-color)' }}>
            ...
            <div className="flex flex-wrap gap-2">
              {selectedUsers.map(user => (
                <div
                  key={user.id}
                  className="flex items-center gap-2 bg-[#2663EB] text-white px-3 py-1 rounded-full text-sm"
                >
                  <Avatar url={user.avatar_url} size={20} />
                  <span>{user.full_name || 'Unknown User'}</span>
                  <button
                    onClick={() => toggleUserSelection(user)}
                    className="text-[#2663EB] hover:text-white"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {loading ? (
            <div className="minimal-flex-center py-8">
              <div className="w-6 h-6 border-2 border-[#2663EB] border-t-transparent rounded-full animate-spin mr-2"></div>
        ...
        <div className="p-4 border-t flex gap-3" style={{ borderColor: 'var(--divider-color)' }}>
          ...
          <button
            ...
            className="flex-1 px-4 py-2 bg-[#2663EB] text-white rounded-lg hover:bg-[#1e4fd4] disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
          >
            {loading ? (
