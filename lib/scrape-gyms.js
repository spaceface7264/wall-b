...
export function isClimbingGym(place) {
  ...
  const description = ((place.editorial_summary?.overview || '') + ' ' + (place.description || '')).toLowerCase();
  const combinedText = `${name} ${description}`.toLowerCase();
  
  // Climbing-related keywords (including variations)
  const climbingKeywords = [
    'klatre', 'klatring', 'klatrehal', 'klatresenter', // Danish
    'climbing', 'boulder', 'bouldering', 
    'rock climb', 'rock climbing', 'rock climbing gym', // Rock climbing variations
    'indoor climbing', 'climbing gym', 'climbing wall',
    'mountain climbing', 'sport climbing',
    'climb', 'wall climbing', 'vertical climbing'
  ];
  
  // Climbing-related Google Places types
  const climbingTypes = [
    'gym', // General gym type (we'll check name/description for climbing keywords)
    'health', // Health-related (might include climbing gyms)
  ];
  
  // Check name and description for climbing keywords
  const hasClimbingKeyword = climbingKeywords.some(keyword => 
    combinedText.includes(keyword)
  );
  
  // If name/description contains climbing keywords, it's likely a climbing gym
  if (hasClimbingKeyword) {
    return true;
  }
  
  // Check if it's a gym type and might be climbing-related
  // If it has 'gym' type, check if there are any climbing indicators
  const hasGymType = types.some(type => climbingTypes.includes(type));
  if (hasGymType) {
    // Additional check: look for climbing-related terms in the combined text
    // This catches cases where Google categorizes it as a gym but name might not be obvious
    const gymKeywords = ['wall', 'route', 'boulder', 'climb', 'klatre'];
    const hasGymClimbingIndicator = gymKeywords.some(keyword => 
      combinedText.includes(keyword)
    );
    if (hasGymClimbingIndicator) {
      return true;
    }
  }
