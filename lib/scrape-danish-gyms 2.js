...
async function createGymRequest(gymRequest, systemUserId) {
  ...
  const requestData = {
    ...
    description: enrichedRequest.description, // Clean description without rating metadata
    facilities: enrichedRequest.facilities || [],
    status: 'pending',
  };

  // Add Google ratings if available (stored separately now)
