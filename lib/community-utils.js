...
/**
 * Checks if a community has new posts since the user's last visit
 * Uses both database and localStorage for optimal performance
 * 
 * @param {string} communityId - Community ID
 * @param {string} userId - User ID
 * @returns {Promise<boolean>} True if there are new posts
 */
export async function checkForNewPosts(communityId, userId) {
  if (!communityId || !userId) return false;

  try {
    // First check localStorage for instant feedback
    const localStorageKey = `community_${communityId}_last_viewed`;
    const lastViewedLocal = localStorage.getItem(localStorageKey);
    
    // Get last_viewed_at from database (fallback to joined_at if null)
    const { data: membership, error: membershipError } = await supabase
      .from('community_members')
      .select('last_viewed_at, joined_at')
      .eq('community_id', communityId)
      .eq('user_id', userId)
      .single();
    ...
    if (membershipError) {
      ...
      if (lastViewedLocal) {
        return checkNewPostsAfterTimestamp(communityId, lastViewedLocal);
      }
      return false;
    }

    // Determine the timestamp to check against
    // Priority: last_viewed_at > joined_at > localStorage > now (no new posts)
    let checkTimestamp = null;
    
    if (membership?.last_viewed_at) {
      checkTimestamp = membership.last_viewed_at;
    } else if (membership?.joined_at) {
      checkTimestamp = membership.joined_at;
    } else if (lastViewedLocal) {
      checkTimestamp = lastViewedLocal;
    } else {
      // First visit - no new posts to show
      return false;
    }

    // Check for posts created after the timestamp
    return checkNewPostsAfterTimestamp(communityId, checkTimestamp);
  } catch (error) {
...
/**
 * Helper function to check if there are posts after a given timestamp
 * 
 * @param {string} communityId - Community ID
 * @param {string} timestamp - ISO timestamp string
 * @returns {Promise<boolean>} True if there are new posts
 */
async function checkNewPostsAfterTimestamp(communityId, timestamp) {
  try {
    const { count, error } = await supabase
      .from('posts')
      .select('*', { count: 'exact', head: true })
    ...
    if (error) {
      console.warn('Error checking posts after timestamp:', error);
      return false;
    }

    return (count ?? 0) > 0;
  } catch (error) {
    console.warn('Error in checkNewPostsAfterTimestamp:', error);
    return false;
  }
}

/**
 * Updates the last_viewed_at timestamp for a user's community membership
 * Updates both database and localStorage for optimal performance
 * 
 * @param {string} communityId - Community ID
 * @param {string} userId - User ID
 * @returns {Promise<void>}
 */
export async function updateLastViewedAt(communityId, userId) {
  ...
  try {
    ...
    if (error) {
      console.warn('Error updating last_viewed_at in database:', error);
      // localStorage already updated, so UI will still work
    }
  } catch (error) {
    console.warn('Error in updateLastViewedAt:', error);
    // localStorage already updated, so UI will still work
  }
}
