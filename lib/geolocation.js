      ...
      (error) => {
        ...
        switch (error.code) {
          ...
          case error.POSITION_UNAVAILABLE:
            // kCLErrorLocationUnknown - location is unavailable (common in dev/simulators)
            // Don't treat this as a critical error - just log it
            console.debug('Location unavailable (this is normal in some environments):', error.message);
          ...
          default:
            ...
            console.debug('Location error (code:', error.code, '):', error.message);
            errorMessage = 'Location information is unavailable.';
        }
...
export async function geocodeAddress(address) {
  ...
  try {
    ...
    const response = await fetch(
      ...
      {
        headers: {
          'User-Agent': 'Send Train Climbing App', // Nominatim requires a user agent
        },
      }
    );
...
export async function reverseGeocode(latitude, longitude) {
  try {
    const response = await fetch(
      ...
      {
        headers: {
          'User-Agent': 'Send Train Climbing App',
        },
      }
    );
