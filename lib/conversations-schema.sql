...
-- Add check constraint for type column (only if column exists)
DO $$ 
BEGIN
...
-- Add check constraint for message_type column (only if column exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
               WHERE table_name = 'direct_messages' AND column_name = 'message_type') THEN
